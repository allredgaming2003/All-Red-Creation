...
import { Mail, ArrowRight, Lock, User, Eye, EyeOff, CheckCircle2, Globe, Shield, KeyRound, Sparkles } from 'lucide-react';
...
export default function GoogleAuthModal({
  ...
  const handleGoogleClick = async () => {
    ...
    if (res.success && res.user) {
      ...
      const isAdmin = res.user.email.toLowerCase() === 'all.red.gaming.2003@gmail.com';
      const userWithAdmin: UserSession = {
        ...res.user,
        isAdmin: isAdmin
      };
  ...
  const completeLogin = (emailToUse: string, nameToUse?: string, provider: 'google' | 'email' = 'google', forcedAdmin?: boolean) => {
    setIsLoggingIn(true);
    ...
    const isAdmin = forcedAdmin || emailToUse.toLowerCase() === 'all.red.gaming.2003@gmail.com';

    const user: UserSession = {
      name: isAdmin ? 'Abhay Kashyap (Admin)' : (formattedName || 'User'),
      email: emailToUse,
  ...
  // Submit Handler for Email or Admin Login
  const handleEmailAuthSubmit = async (e: React.FormEvent) => {
    ...
    // ADMIN LOGIN MODE VALIDATION
    if (isAdminMode) {
      if (cleanEmail.toLowerCase() === 'all.red.gaming.2003@gmail.com' && passwordInput === '722009') {
        completeLogin('all.red.gaming.2003@gmail.com', 'Abhay Kashyap (Admin)', 'email', true);
      } else {
        setErrorMessage('Invalid Admin Credentials! Admin login is strictly allowed for owner email (all.red.gaming.2003@gmail.com) with correct password.');
      }
      return;
    }
    ...
    if (isSignUp) {
      ...
      if (res.success) {
        ...
        setAuthMode('signin'); // Switch client to Sign In view
        setOtpSent(false);
      ...
      // Check if trying to sign in with admin credentials via main form
      if (cleanEmail.toLowerCase() === 'all.red.gaming.2003@gmail.com' && passwordInput === '722009') {
        completeLogin('all.red.gaming.2003@gmail.com', 'Abhay Kashyap (Admin)', 'email', true);
        return;
      }
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
        ...
        <motion.div
          ...
          className={`relative w-full max-w-md bg-[#0d0d10] border rounded-2xl p-6 sm:p-8 shadow-2xl overflow-hidden text-gray-100 ${
            isAdminMode ? 'border-brand-red/50 shadow-[0_0_30px_rgba(255,0,0,0.25)]' : 'border-white/10'
          }`}
        >
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-display font-bold text-white uppercase tracking-tight">
              {isAdminMode ? (
                <>ADMIN <span className="text-brand-red">OWNER</span> LOGIN</>
              ) : (
            ...
            <p className="text-xs text-gray-400 mt-1 font-sans">
              {isAdminMode 
                ? 'Authorized Owner Access (all.red.gaming.2003@gmail.com)' 
                : isSignUp 
                  ? 'Create an account to access client projects' 
                  : 'Sign in to access your video agency portal'
              }
            </p>
          </div>
          ...
          {isLoggingIn ? (
            <div className="py-8 text-center space-y-3 bg-white/5 border border-white/10 rounded-xl my-4">
              ...
              <p className="text-[10px] text-gray-400 font-mono">Connecting to dashboard...</p>
            </div>
          ) : (
            <>
              {/* Primary Google Sign-In Button (Hidden in Admin Mode) */}
              {!isAdminMode && (
              ...
              <form onSubmit={handleEmailAuthSubmit} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-[10px] font-mono text-gray-400 mb-1 uppercase flex items-center justify-between">
                    <span>{isAdminMode ? 'Owner Email Address' : isSignUp ? '2. Email Address' : 'Email Address'}</span>
                    {!isAdminMode && emailValidation && (
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full bg-black/40 border border-white/15 rounded-lg py-2.5 pl-9 pr-4 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-brand-red transition-all"
                    />
                  </div>
                </div>

                {/* OTP Verification Section for Sign Up */}
                {isSignUp && (
                  <div className="space-y-2.5">
                    {!isOtpVerified ? (
                      <div className="p-3 bg-white/5 border border-white/10 rounded-xl space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] font-semibold text-gray-300">Email Verification OTP</span>
                          {!otpSent && (
                            <button
                              type="button"
                              onClick={handleSendOtp}
                              disabled={sendingOtp || !emailInput || !nameInput}
                              className="px-3 py-1 bg-brand-red hover:bg-brand-red-dark disabled:opacity-50 text-white font-bold text-[10px] uppercase rounded transition-colors cursor-pointer"
                            >
                          ...
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-lg transition-colors cursor-pointer"
                              >
                                Verify OTP
                              </button>
                            </div>
                      ...
                      <div className="p-2.5 bg-emerald-500/15 border border-emerald-500/40 rounded-xl flex items-center gap-2 text-xs text-emerald-400 font-semibold">
                        ...
                        <span>Email Verified! Set your password below.</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-[10px] font-mono text-gray-400 uppercase">
                      {isAdminMode ? 'Admin Password' : isSignUp ? '3. Create Password' : 'Password'}
                    </label>
                  </div>
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl text-white text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-lg mt-2 ${
                    isAdminMode
                      ? 'bg-brand-red hover:bg-red-700 shadow-brand-red/30'
                      : 'bg-brand-red hover:bg-brand-red-dark shadow-brand-red/20'
                  } ${isSignUp && !isOtpVerified ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isAdminMode ? (
                    <>
                      <Shield className="w-4 h-4" />
                      <span>{isLoggingIn ? 'Authenticating Admin...' : 'Login as Owner Admin'}</span>
                    </>
                  ) : (
                    <>
                      <span>{isLoggingIn ? 'Connecting...' : isSignUp ? 'Create Account' : 'Sign In'}</span>
          ...
          <div className="mt-5 pt-4 border-t border-white/10 space-y-3">
            ...
            {/* ADMIN LOGIN OPTION LOCATED BELOW SIGN UP OPTION */}
            {!isAdminMode && (
              <div className="pt-2 border-t border-white/5 flex items-center justify-between text-xs text-gray-400">
                <span className="text-[11px] font-mono text-gray-500">Agency Owner Access</span>
                <button
                  type="button"
                  onClick={() => {
                    setAuthMode('admin');
                  ...
                  className="px-2.5 py-1 rounded bg-brand-red/10 hover:bg-brand-red/20 border border-brand-red/30 text-brand-red font-semibold transition-all cursor-pointer flex items-center gap-1 text-[11px]"
                >
